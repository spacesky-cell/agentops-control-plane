...
The unscoped npm package `agentpermit` is the primary distribution. It has no JavaScript runtime dependencies and no install scripts. `bin/agentpermit.js` locates Python in this order: `AGENTPERMIT_PYTHON`, Windows `py -3`, `python3`, then `python`; it requires Python 3.10+ and starts an isolated `-I -c` bootstrap that inserts only the bundled package root ahead of the interpreter's safe paths before running `agentpermit` with `runpy`. The launcher preserves the current working directory, inherited stdio, user arguments, and child exit status. On POSIX it starts Python in a detached process group and forwards every `SIGINT`, `SIGTERM`, or `SIGHUP` received while the child is active to that group, allowing repeated interrupts and escalation. On Windows it keeps Python in the shared console process group and relies on console signal delivery instead of explicitly signaling the child. Signal handlers are removed on child error or exit.
