...
AgentPermit 是一个面向 AI Agent 的轻量级运行治理层。它不试图替代 Codex、Claude Code、LangGraph 或 OpenAI Agents SDK，而是位于这些 Agent 后端之上，统一管理工具执行、权限策略、人工审批、审计日志、隔离工作区、快照和运行报告。
...
- 提供标准 MCP JSON-lines 服务，支持 `initialize`、`tools/list` 和受治理的
  `tools/call`。
- 支持 HTML / JSON 运行报告导出。
...
只有在本地服务进程可信时才使用 `--auto-approve`。MCP 客户端不能自行开启
自动审批。完整协议和审批重试流程见 [docs/MCP_STDIO.md](docs/MCP_STDIO.md)。
...
```powershell
python -m agentpermit export <run_id> --format json --out report.json
```
...
原始 `examples/sample_repo` 不会被修改。你可以通过 `.agentpermit/workspaces/<run_id>` 查看 Agent 实际修改后的隔离工作区。

## 公开仓库内容说明

这个仓库面向公开展示，刻意保留了源码、文档、示例和 `tests/` 测试源码。`tests/` 不是测试运行产物，它的作用是让招聘方或 reviewer 能验证项目质量。

不会提交运行生成物和本地隐私文件。`.agentpermit/`、`.pytest_cache/`、`__pycache__/`、导出的 demo 报告、本地环境文件和日志都已经写入 `.gitignore`。
...
- `agentpermit/gateway.py`：所有工具调用的统一入口。
- `agentpermit/policy.py`：策略判断和风险分级。
- `agentpermit/tools.py`：受控工具执行器。
- `agentpermit/workspace.py`：隔离 workspace 和快照管理。
- `agentpermit/audit.py`：SQLite 审计存储。
- `agentpermit/agents.py`：deterministic scripted agent adapter。
- `agentpermit/web.py`：本地 Dashboard。
- `agentpermit/evaluator.py`：批量 eval 入口。
...
```powershell
python -m agentpermit eval --tasks examples\tasks.jsonl --auto-approve
```
...
> Built AgentPermit, a vendor-neutral control plane for AI agents with isolated workspaces, policy-based tool execution, approval gates, command/file audit logs, snapshots, trace export, and deterministic evaluation demos.
...
- 增加更丰富的 MCP resource 和 prompt 能力。
