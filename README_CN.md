...
启动器需要 Node.js 18+ 和 Python 3.10+，没有 npm 依赖或安装钩子。
...
第一次 `tools/call` 会创建受治理运行。被策略拦截的调用会返回稳定的待审批 id；在 Dashboard 或执行 `agentpermit --home .demo approve <approval_id>` 批准后，重试完全相同的 MCP 调用。
...
```powershell
claude mcp add --scope project agentpermit -- npx --yes agentpermit@0.3.0 --home . mcp --source . --task "治理此工作区"
```
...
command = "npx"
args = ["--yes", "agentpermit@0.3.0", "--home", ".", "mcp", "--source", ".", "--task", "治理此工作区"]
```
...
agentpermit --home .demo approve <approval_id> --approver reviewer --reason "已审核准确请求"
agentpermit --home .demo reject <approval_id> --approver reviewer --reason "已拒绝准确请求"
