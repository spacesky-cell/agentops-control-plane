...
![AgentPermit 已完成运行，展示创建、修改和删除文件的快照证据](docs/assets/dashboard-completed-run.png)
...
```powershell
claude mcp add --scope project agentpermit -- agentpermit --home . mcp --source . --task "治理此工作区"
```

Codex（项目本地配置）：

```powershell
codex mcp add agentpermit -- agentpermit --home . mcp --source . --task "治理此工作区"
```

如果客户端找不到 npm bin 目录，可以将 `agentpermit` 替换为绝对路径。不要把 `--auto-approve` 放进客户端配置；它只适用于明确可信的本地演示服务进程。
...
命令行等价操作包括 `agentpermit approvals`、`agentpermit approve <id> --approver <name> --reason <text>`、`agentpermit reject ...`、`agentpermit show <run_id>` 和 `agentpermit export <run_id> --format html --out report.html`。
